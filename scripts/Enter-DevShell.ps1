...
{
    $preferredShell = "C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\Common7\Tools\Launch-VsDevShell.ps1"

    if (Test-Path -LiteralPath $preferredShell) {
        return Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $preferredShell))
    }
